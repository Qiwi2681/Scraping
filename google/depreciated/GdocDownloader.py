...
# Create a zip archive and compress the downloaded files
...
with zipfile.ZipFile(output_zip_file, 'w', zipfile.ZIP_DEFLATED) as zipf:
    for file_id in file_ids:
        try:
            ...
            zipf.writestr(f"resume_{file_id}.rtf", file_content.getvalue())

            print(f"File '{file_metadata['name']}' downloaded and compressed")

        except HttpError as e:
            # Handle the HttpError, for example, print the error message
            print(f"Error downloading file with ID '{file_id}': {e}")

print("All files downloaded and compressed successfully into 'compressed_docs.zip'.")
